...
from coupons.views import coupon_create, coupon_deactivate, coupon_delete
...
def cart_add(request, product_id):     
    ...
    if product.discount is not None and product.discount > 0:
        try:
            coupon_delete(request=request)
            logging.info(
                "Product discount has been in the cart which violates the usages of coupon except Darkove certifikaty."
            )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                logging.error(f"Serialization error - this is a very unique error: {e}")
                return redirect("cart:cart_detail")
...
def update_cart_country(request, online=None):
    ...
    if cart.get_shipping() == "S":
        if request.method == "POST":
            ...
            request.session["cart_country"] = "CZ"
            request.session["cart_address"] = "U Vodárny 1225/4"
            request.session["cart_vendor"] = "-"
            request.session["cart_shipping"] = "S"
            request.session["cart_city"] = "Praha 3"
            request.session["cart_zipcode"] = "13000"
            # request.save()
