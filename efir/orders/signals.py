...
def send_email_when_order_created(sender, instance, created, **kwargs):
    ...
    if created and not instance.confirmation_sent:
        ...
        try:
            ...
            logger.error(
                f"There has been an error while triggering signal and sending an confirmatory email in order {instance.id}, error: {e}"
            )
...
def send_email_when_order_completed(sender, instance, created, **kwargs):
    ...
    if instance.shipped and not instance.shipped_sent:
        ...
        # Update the 'shipped_sent' field without triggering the signal again
        Order.objects.filter(pk=instance.pk).update(shipped_sent=True, status="V")
...
def send_email_when_order_paid(sender, instance, **kwargs):
    ...
    if instance.paid and not instance.paid_confirmation_sent:  # TODO vlk test this
        ...
        OrderStatusService.mark_as_paid(instance)
